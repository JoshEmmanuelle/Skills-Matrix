...
def _free_tier_notice() -> str:
    return (
        ...
        "If you need deeper or high-volume analysis, use company's Microsoft Copilot Chat, or use the Agent listed on the instructions under Option 3"
    )
...
def ask_llm(question: str, context: str) -> str:
    ...
    try:
        ...
        return (
            "Rate limit reached for MasterPeace GitHub Models Access, please try again. Use company's Microsoft 365 Copilot, or our SMRP Agent.\n\n"
            + _free_tier_notice()
        )
        ...
        return (
            ...
            "Please try a shorter question or wait and try again. Also, you can use company's Microsoft 365 Copilot, or our SMRP Agent.\n\n"
            + _free_tier_notice()
        )

    except Exception:
        return (
            "The AI service encountered an unexpected error, please try again. Use company's Microsoft 365 Copilot, or our SMRP Agent.\n\n"
            + _free_tier_notice()
        )
...
def render_llm_badge(where: str = "sidebar") -> None:
    ...
    if where == "sidebar":
        if enabled:
            ...
            st.sidebar.caption("MasterPeace LLM access: GitHub Models may be rate-limited. For deeper analysis use company's Microsoft Copilot Chat, or our SMRP Agent under Option 3")
        else:
            st.sidebar.warning(badge_text)
    else:
        if enabled:
            st.success(badge_text)
            st.caption("MasterPeace LLM access: GitHub Models may be rate-limited. For deeper analysis, use company's Microsoft Copilot Chat, or our SMRP Agent under Option 3")
        else:
            st.warning(badge_text)


def render_chat(excel_bytes: bytes, state_key_prefix: str = "chat") -> None:
    """Render chatbot UI for the generated workbook only.

    - Chatbot always uses the Excel produced by this app run
    - No external or user-uploaded workbooks allowed
    - Keeps chatbot strictly read-only and deterministic
    """
    ...
    with st.expander("**Recommended ways to ask questions about the Excel file**", expanded=False):
        st.markdown(
        """
**The chatbot answers questions using ONLY the Excel generated by this app.**  
It does **not** infer, guess, or recommend people.

### ✅ Good example questions
- *How many candidates have Kubernetes experience?*
- *Which certifications appear most frequently across all candidates?*
- *List candidates who have AWS skills.*
- *How many candidates have a Bachelor’s degree vs a Master’s degree?*
- *Does anyone list both Docker and Kubernetes?*
- *Show the top 10 most common skills.*

### ✅ Comparative / summary questions
- *What are the most common certifications?*
- *How many candidates list Python?*
- *Which skills appear in more than 5 candidates?*
- *What degrees are most common?*

### ❌ Questions the chatbot will NOT answer
- *Who is the best candidate?*
- *Which candidate should we hire?*
- *Who is most senior or most experienced?*
- *Who would be a good fit for role X?*
- *Does candidate A know technology B if it is not listed?*

These questions are intentionally unsupported to ensure **deterministic, rule‑compliant behavior**.
"""
    )
    ...
    # ✅ Always use the generated output from this session
    active_bytes = excel_bytes
    st.caption("Using the Excel output generated by this session")
    ...
    user_q = st.chat_input(
        "Ask a question about the Skills Matrix just generated "
        "(e.g., who has Kubernetes, top certifications, degree counts)"
    )
